...
const UsersCRUD = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("http://3.110.174.202:8081/users");
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://3.110.174.202:8081/users", form);
  ...
  const handleUpdateUser = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://3.110.174.202:8081/users/${form.id}`, form);
  ...
  const handleDeleteUser = async (id) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        await axios.delete(`http://3.110.174.202:8081/users/${id}`);
